...
class Cycloid(object):
    ...
    def initiate_split(self, orbitmax):
        ...
        if s1 > self.tensile_str:
            ...
            CycloidCoords.append((self.start_lon, self.start_lat))
    ...
    def plotcoordsonbasemap(self,basemap_ax, orbit_pos, triangles):
        """
...
def SaveCycloidAsShape(filename):
    ...
    cycloidline = ogr.Geometry(ogr.wkbMultiPoint)
    #cyc = Cycloid(stresscalc, threshold, propagation_strength, propagation_speed, start_lon, start_lat, start_dir, 0.1, vary_velocity, k)
    for point in CycloidCoords:
        Point1 = ogr.Geometry(ogr.wkbPoint)
        x, y = point
        Point1.AddPoint(x, y)
        cycloidline.AddGeometry(Point1)
    ...
    outLayer = outDataSource.CreateLayer(str(filename), geom_type=ogr.wkbMultiPoint)
