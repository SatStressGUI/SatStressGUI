...
class Cycloid(object):
    ...
    def __init__(self, stresscalc, threshold, propagation_strength, propagation_speed,
                       start_lat, start_lon, start_dir='West', degree_step=0.5,
                        varyVelocity=False, k=0):
        """
        ...
        self.tensile_str = threshold*1000 # kPa
        self.propagation_strength = propagation_strength*1000   # kPa
        self.propagation_speed = propagation_speed         # m/s
    ...
    def set_interval(self, degree):
        """
        Calculates the time interval in seconds for finding angular distance
    ...
    def calc_ang_dist(self):
        ...
        linear_distance = propagation_speed * time_step,
    ...
    def get_stresses(self, lat, lon, time):
        """
        ...
        """
        return self.stresscalc.principal_components(pi/2 -lat, lon, time)
    ...
    def initiate_split(self, orbitmax, time=0.0):
        """
        Initiates, or fails to initiate a crack at the starting location. {Orbitmax} is included
        for passing into propagate_split(). 
        Since the value of the stress components depends on the {time} elapsed since periapsis,
        this value is also included as a parameter.
        
        """

        # get stress components
        s1, s1az, s3, s3az = self.get_stresses(radians(self.start_lat), radians(self.start_lon), time)

        print "s1, tensile", float(s1), self.tensile_str, "types: ", type(float(s1)),type(self.tensile_str), float(s1)> float(self.tensile_str)
        ...
        if s1 > self.tensile_str:
            ...
            pt_data = (self.start_lat, self.start_lon, s1, s1az, s3, s3az)
            self.data.append(pt_data)

            self.propagate_split(pt_data, orbitmax, time + self.time_step)
    ...
    def propagate_split(self, prevpt_data, orbitmax, time):
        """
        ...
        while self.traveled <= orbitmax:
            ...
            assert(self.lat_cycloidpts[-1] == prevlat)
            assert(self.lon_cycloidpts[-1] == prevlon)

            ##### Find next point #####

            # if varyVelocity option is set
            if self.varyVelocity == True:
                self.ang_dist = vary_ang_dist(s1mag)
                print self.ang_dist
            ...
            (new_s1, new_s1az, new_s3, new_s3az) = self.get_stresses(newlat, newlon, time)
            ...
            if new_s1 > self.propagation_strength:
                ...
                self.lon_cycloidpts.append(dnewlon)
                # update data list
                pt_data = (dnewlat, dnewlon, new_s1, new_s1az, new_s3, new_s3az)
                self.data.append(pt_data)
                ...
                # failed to propagate at location {prevlat, prevlon} at {time},
                # but may propagate at same location at future {time}, so
                # increment time and orbit traveled, but not lat, lon
                time += self.time_step
                self.traveled += self.degree_step
...
def plotCycloid(cyc, orbit_pos, basemap_ax):
    """
    Plots onto a given basemap the cycloids generated from periapsis until {time}.
    In order to be efficient and save time from calculations, the cycloid calculation
    will be done in 'real time'. Recalculation will be avoided. Specifically, cycloid
    points that are already generated will not be recalculated, only plotted. If the
    cycloid points have been calculated only up to time t_o, and {time} > t_o, then 
    ...
    if (len(cyc.lat_cycloidpts) == 0) and (orbit_pos != 0):
        ...
        cyc.propagate_split(cyc.lat_cycloidpts[-1], cyc.lon_cycloidpts[-1], orbit_pos, t)
        plot2basemap()
...
def plotcoordsonbasemap(stresscalc, basemap_ax,
    ...
    cyc = Cycloid(stresscalc, threshold, propagation_strength, propagation_speed, start_lon, start_lat, start_dir, 0.1)
