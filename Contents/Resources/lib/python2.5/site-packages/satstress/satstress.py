...
class PW_Variables(object): #
  ...
  def __init__(self, thetaRi=0, phiRi=0, thetaRf=0, phiRf=0, thetaTi=0, phiTi=0, thetaTf=0, phiTf=0, locked=True, despinning=False, InitialSpin=0, FinalSpin=0):
  ...
  def update_InitialSpin(self, InitialSpin):
    self.InitialSpin = InitialSpin

  def update_FinalSpin(self, FinalSpin):
    self.FinalSpin = FinalSpin
...
class PolarWander(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        if self.PWCoordinates.despinning:
          ...
          RotationRate = (2.0 * scipy.pi) / self.satellite.orbit_period()
          #For a tidally locked body in synchronous rotation, the rotation rate will be the same as the orbit rate.
          self.Flattening = ((self.satellite.radius())**3 * RotationRate**2) / (2.0 * pc.G * self.satellite.mass()) * self.love.h2.real          
        ...
        Unused code for allowing the user to step through how the stress changes.
        This is not implemented because I did not have time and we are using an elastic model, so it is time-independent.
        """
    ...
    def Ttt(self, theta, phi, t):
        ...
        RDi = self.PWCi * (6 * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri))**2) + (9 * self.CosGamma(theta, phi, theta_Ri, phi_Ri)**2) - 5 #Calculates the stress from the initial position of the pole
        RDf = self.PWCf * (6 * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf))**2) + (9 * self.CosGamma(theta, phi, theta_Rf, phi_Rf)**2) - 5 #Calculates the stress from the reorientation of the pole
        TDi = self.PWCi * (6 * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti))**2) + (9 * self.CosGamma(theta, phi, theta_Ti, phi_Ti)**2) - 5 #Calculates the stress from the initial tidal deformation
        TDf = self.PWCf * (6 * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf))**2) + (9 * self.CosGamma(theta, phi, theta_Tf, phi_Tf)**2) - 5 #Calculates the stress from the final tidal deformation
    ...
    def Tpp(self, theta, phi, t):
        ...
        RDi = self.PWCi * (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Ri, phi_Ri))**2) + 1.0 #Calculates the stress from the initial position of the pole
        RDf = self.PWCf * (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Rf, phi_Rf))**2) + 1.0 #Calculates the stress from the reorientation of the pole
        TDi = self.PWCi * (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Ti, phi_Ti))**2) + 1.0 #Calculates the stress from the initial tidal deformation
        TDf = self.PWCf * (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Tf, phi_Tf))**2) + 1.0 #Calculates the stress from the final tidal deformation   
    ...
    def Tpt(self, theta, phi, t):
        ...
        RDi = self.PWCi * 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Ri, phi_Ri)) * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri)) #Calculates the stress from the initial position of the pole
        RDf = self.PWCf * 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Rf, phi_Rf)) * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf)) #Calculates the stress from the reorientation of the pole
        TDi = self.PWCi * 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Ti, phi_Ti)) * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti)) #Calculates the stress from the initial tidal deformation
        TDf = self.PWCf * 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Tf, phi_Tf)) * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf)) #Calculates the stress from the final tidal deformation   
