...
class StressDef(object): #{{{
    ...
    dependson = set(['DENSITY',\
                     ...
                     'THICKNESS',\
                     'VISCOSITY'])
    # Thse variables of each subclass 
...
class DiurnalObliquity(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        @param satellite: the satellite to which the stress is being applied.
        @type satellite: L{Satellite}
        @return: an object defining the NSR stresses for a particular satellite. <-- WAIT huh?
        @rtype: L{NSR}
        """
        ...
        """self.n = self.satellite.mean_motion()
        self.rho_av = avg_body_density(self.satellite)
        a = self.satellite.orbit_semimajor_axis
        M = self.satellite.planet_mass
        mu = self.satellite.layers[3].lame_mu # rigidity of the shell
        nu = self.satellite.layers[3].poissons_ratio
        self.k = 3 * M * mu * numpy.abs(self.love.h2) / (8 * numpy.pi * self.rho_av * numpy.power(a, 3)) \
                    * ((1 + nu) / (5 + nu))"""
    ...
    ### We define the velocity of the crack in m/s
    ### Cracking velocity is usually between 1 and 10 m/s, while crack constant of proportionality shoud be set below 10e-4
    """def V(self, crack_speed, crack_const, stress, crack_init):"""
    """
        Crack propagation velocity from Lingen (2004)
    """
    """V = crack_speed + crack_const*(stress-crack_init)
        #delta = theta
        #lambda_ = -phi
        return V
        #return self.Sxy_inv(delta, lambda_)"""
...
# Ben's note: This PolarWander class is EXPERIMENTAL!!! I added some stuff and it didn't work, so I've removed it...
class PolarWander(StressDef): #{{{
    ...
    # these values may change
    theta_rot = scipy.pi/2.0 # colatitude of rotation axis
    phi_rot = 0 # longitude of rotation axis
...
class StressCalc(object): #{{{
    ...
    def principal_components(self, theta, phi, t): #{{{2
        ...
        !! Note input parameter theta is co-latitude, no latitude
